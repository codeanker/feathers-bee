...
class Service {
  ...
  setup () {
    ...
    this.queue.on('succeeded', (job, result) => this.emit('succeeded', { job, result }));
    this.queue.on('retrying', (job, error) => this.emit('retrying', { job, error }));
  ...
  _jobProcess (job, done) {
    return this.service.create(job.data, {$jobId: job.id})
